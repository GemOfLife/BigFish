...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    Piece movedPiece;
    int moveCount, captureCount, quietCount, improvement=0;
    ...
    if (   !ss->ttPv
        ...
        return eval - futility_margin(depth, cutNode && !ss->ttHit, improving) - (ss-1)->statScore / 306;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doDeeperSearch = value > (abs(bestValue - ss->staticEval) < 32 ? ss->staticEval : bestValue) + 64 + 11 * (newDepth - d);
          ...
          if (value > alpha)
          {
              ...
              else
              {
                  ...
                  if (   depth > 2
                      ...
                      && beta  <  14907
                      && value > -11683)
                      depth -= (ss-1)->moveCount < 3 && ss->moveCount < 2 && depth < 6 ? 3 : 2;
